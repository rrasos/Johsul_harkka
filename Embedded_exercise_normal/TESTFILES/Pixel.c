...
void setup() {
    ...
    // Resetting screen at start is a MUST for operation (Set RST-pin to 1).
    ...
    CONTROL_SIGNAL |= (1 << 0); // bit 0 to 1 again
    ...
    // Write code that sets 6-bit values in the register of the DM163 chip.
...
void run(uint8_t x) {
    ...
    for (uint8_t y = 0; y < 8; y++) { // Loop through rows
        ...
        for (uint8_t color = 0; color < 3; color++) { // Loop through color channels (B, G, R)
            ...
            uint8_t pixel_data = dots[x][y][color];



            for (uint8_t bit_pos = 0; bit_pos < 8; bit_pos++) { // Loop through 8 bits
                ...
                pixel_data <<= 1; // Shift to the next bit
